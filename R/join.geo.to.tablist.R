...
#' @export
join.geo.to.tablist <- function(mygeo, my.list.of.tables, save.csv=FALSE, sumlevel='both', folder=getwd(), testing=FALSE, end.year='') {
...
# But it may be useful to have one file per table.
	...
	for(i in 1:length(my.list.of.tables)) {
		...
		if (save.csv) {
			...
			write.csv(my.list.of.tables[[i]], file=file.path(folder, paste("ACS", end.year, "-", this.tab, ".csv", sep="")), row.names=FALSE)
