...
download.datafiles <- function(tables,
  ...
  # stateinformation <- EJAM::stateinfo2
  # stateinformation <- ejanalysis::get.state.info() # which did  data(lookup.states, package='proxistat')
  stateinformation <- proxistat::lookup.states
  ...
  for (statenum  in 1:length(mystates)) {
    ...
    state.abbrev <-
      tolower(stateinformation$ST[tolower(state.abbrev) == tolower(stateinformation$ST)])
    ...
    state.name 	<-
      stateinformation$ftpname[tolower(state.abbrev) == tolower(stateinformation$ST)]
