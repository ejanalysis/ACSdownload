...
get.field.info <- function(tables, end.year='2012', table.info.only=FALSE, moe=FALSE) {
  ...
  if (table.info.only) {
    if (moe) {
      ...
      table.info$title <- paste("MOE for", table.info$title, sep=' ')
    }
  } else {
    ##########
    ...
    field.info$tab.var <- paste(field.info$Table.ID, analyze.stuff::lead.zeroes(field.info$Line.Number,3), sep='.')
    ...
    field.info <- data.frame(table.ID=field.info$table.ID, line=field.info$line.number,
                             ...
                             table.title=field.info$table.title, universe=field.info$universe, subject=field.info$subject,
                             stringsAsFactors=FALSE)
