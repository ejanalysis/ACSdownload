...
read.concat.states <- function(tables, mystates, geo, needed, folder=getwd(), output.path, end.year='2012', save.files=TRUE, sumlevel='both', testing=FALSE, dt=TRUE) {
  ...
  if (!dt) {
    if (missing(geo)) {
      ...
      states.num.places <- data.frame(STUSAB=mystates.no.us, count=2400+8100, stringsAsFactors = FALSE)
    } else {
      # to find data file length (nrow), use state abbrev's list
      states.num.places <- as.data.frame(table(geo$STUSAB[geo$STUSAB %in% toupper(mystates.no.us)]))
  ...
  for (this.seq in seqfilelistnums.mine) {
    ...
    if (!dt) {
      ...
      cols.in.csv <- length(read.csv(file.path(folder, efiles.not.us[1]), nrows=1, header=FALSE))
    } else {
      cols.in.csv <- length( data.table::fread(input=file.path(folder, efiles.not.us[1]), nrows=0) )
    }
    ...
    for (this.tab in mytables.in.this.seq) {
      ...
      if (dt) {
        ...
        bigtable.e <- data.table::rbindlist(
          lapply(file.path(folder, fileNames), 
                 function(x) {data.table::fread(x, stringsAsFactors=FALSE, header=FALSE,  colClasses=my.colClasses, na.strings=".")}
          ) 
        ) #
        ...
        for (this.file in efiles.not.us) {
          ...
          this.data <- read.csv(file.path(folder, this.file), stringsAsFactors=FALSE, header=FALSE,
                                comment.char = "",
                                nrows=rows.here,
                                colClasses=my.colClasses,
                                na.strings=".")
      ...
      if (dt) {
        ...
        bigtable.m <- data.table::rbindlist(lapply(
          file.path(folder, fileNames), 
          function(x) {data.table::fread(x, stringsAsFactors=FALSE, header=FALSE, colClasses=my.colClasses, na.strings=".")})
        )
      ...
      if (dt) {
        data.table::setnames(bigtable.e, names(bigtable.e), c( needed.keycols, needed.varnames))
        data.table::setnames(bigtable.m, names(bigtable.m), c( needed.keycols, needed.varnames))
      } else {
