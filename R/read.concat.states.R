...
#' @details This is a slow function using rbind() -- It can takes several minutes for a national dataset of a few tables. 
#'   Also note some sequence files might have >1 table in the sequence file, 
#'   so must loop through tables not seqfiles to get start points right.
...
read.concat.states <- function(tables, mystates, geo, needed, folder=getwd(), end.year='2012', save.files=TRUE, sumlevel='both', testing=FALSE) {
  ...
  for (this.seq in seqfilelistnums.mine) {
    ...
    mytables.in.this.seq <- mytables.in.this.seq[mytables.in.this.seq %in% tables]	
    ...
    for (this.tab in mytables.in.this.seq) {
      ...
      if (save.files) { 
        ...
        save(file.path(folder, bigtable), file=fname)
