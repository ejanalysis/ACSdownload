...
#'
#' Note: 11-character inputs are ambiguous -- they could be a complete
#' tract or a blockgroup missing its leading zero. This implementation
#' leaves them alone and emits a warning, matching the behavior of
#' EJAM::fips_lead_zero() prior to its own ambiguity-resolution logic.
#'
#' @param fips character vector of fips codes
#' @param quiet logical; if FALSE, warn about how many inputs were NA-ed
#' @returns character vector of length-normalized fips codes
#' @seealso EJAM::fips_lead_zero() for a version that resolves 11-char ambiguity
#' @keywords internal
#' @noRd
fips_lead_zero_acs <- function(fips, quiet = TRUE) {
  ...
  if (11 %in% lens) {
    warning(
      "ambiguous fips specified - 11 digits means could be tract if NOT ",
      "missing a leading zero or blockgroup missing leading zero"
    )
  }
