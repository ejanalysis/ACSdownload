...
#'   
#'   Default is 'all' which is 50 states plus DC plus PR (but not AS, GU, MP, UM, VI, US).
...
#'   One element of the vector can be 50, 51, or 52, which represents the 50 states, those 50 plus DC, or the 51 plus PR. 
...
clean.mystates <- function(mystates='all', testing=FALSE) {
  ...
  if (50 %in% mystates) {mystates <- c(mystates[mystates!=50], allabbs[1:51]); mystates <- mystates[mystates!='dc']}
  if (51 %in% mystates) {mystates <- c(mystates[mystates!=51], allabbs[1:51])}
  if (52 %in% mystates) {mystates <- c(mystates[mystates!=52], allabbs[1:51], 'pr')}
