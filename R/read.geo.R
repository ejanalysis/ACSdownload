...
#' @export
read.geo <- function(mystates, folder=getwd(), end.year='2012', silent=FALSE) {
  ...
  #     save(geoformat2014, file = 'geoformat2014.RData')
    ...
    ####################################################################
  geoformat <- switch(paste('x', end.year, sep=''),
                      x2009 = geoformat2009,
                      x2010 = geoformat2010,
                      x2011 = geoformat2011,
                      x2012 = geoformat2012,
  ...
  for (this.file in gfiles.not.us) {
    ...
    #cat(this.file)
    if (!silent) {cat(mystates[mystates!='us'][i], ' ')}
    i <- i + 1
    ...
    this.data <- read.fwf(file.path(folder, this.file),
                          widths=getwidths(geofields, geoformat), as.is=TRUE, header=FALSE,
