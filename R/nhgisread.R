...
#' @export
nhgisread <- function(datafile, codebookfile=gsub("\\.csv", "_codebook.txt", datafile), folder=getwd() ) {
  ...
  datafile.combo <- datafile[!(datafile %in% c(datafile.E, datafile.M))]
  codebookfile.E <-  grep('E_codebook', codebookfile, value = TRUE)
  codebookfile.M <-  grep('M_codebook', codebookfile, value = TRUE)
  codebookfile.combo <- codebookfile[!(codebookfile %in% c(codebookfile.E, codebookfile.M))]
  ...
  if (length(datafile.E) > 1) {stop('Cannot read more than one estimates file at a time - nhgisread is not vectorized')}
  if (length(datafile.M) > 1) {stop('Cannot read more than one MOE file at a time - nhgisread is not vectorized')}
  if (length(datafile.combo) > 1) {stop('Cannot read more than one combined Est/MOE data file at a time - nhgisread is not vectorized')}
  if (length(codebookfile.E) > 1) {stop('Cannot read more than one estimates codebook file at a time - nhgisread is not vectorized')}
  if (length(codebookfile.M) > 1) {stop('Cannot read more than one MOE codebook file at a time - nhgisread is not vectorized')}
  if (length(codebookfile.combo) > 1) {stop('Cannot read more than one combined Est/MOE codebook file at a time - nhgisread is not vectorized')}

  if (length(datafile.E) > 0) {
    Efile <- TRUE # an Estimates-only file exists and name was passed here (was passed here with or without MOE filename also)
    if (length(datafile.combo) > 0 ) {stop('cannot read estimates file and non-MOE file like combined E/MOE file at same time')}
  ...
  if (length(datafile.M) > 0) {
    Mfile <- TRUE # an MOE-only file exists and name was passed here (with or without its Estimates filename as well)
    if (length(datafile.combo) > 0 ) {stop('cannot read MOE file and non-estimates file like combined E/MOE file at same time')}
    if (!file.exists(file.path(folder, codebookfile.E)) ) {
      stop("Codebook file ", codebookfile.E, " not found in ", folder)
    }
  } else {
  ...
  if (Efile & Mfile) {
    ...
    if (!(datafile.E == gsub("M\\.csv", "E\\.csv", datafile.E)) ) {stop('if E and MOE filenames specified they must match except for E or M letter')}
    getting.EM <- TRUE
  ...
  cat(paste("Datafile: ", datafile.E, datafile.M, datafile.combo,sep=' ')); cat("\n")
  cat(paste("Codebook: ", codebookfile.E, codebookfile.M, codebookfile.combo,sep=' ')); cat("\n")
  cat("----------------------"); cat("\n")
  #print(Sys.time())
  ...
  if (getting.EM)  {
    ...
    acs <- acs[ match(acs$GISJOIN, orig.order), ] # put back in original order just in case merge caused problems
  }

  #print(Sys.time())
  cat("Done reading datafile(s)"); cat("\n")
  cat("----------------------"); cat("\n")
...
cat('filename of codebook to read now: ', codebookfile[1])
  ...
  if (is.null(x)) { stop("Codebook or folder missing or not valid")}
  ...
  geolevel	<- x$geolevel  # this assumes a file has only one geolevel in it ******
  years		<- x$years
  dataset		<- x$dataset
