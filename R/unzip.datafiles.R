...
unzip.datafiles <-
  function(tables,
    ...
    for (st in mystates) {
      zipnames <-
        c(zipnames, zipfile(st, myseqfiles, end.year = end.year))
    }
    zipnames <- zipnames[-1]
    ################### #
    ...
    for (statenum  in 1:length(mystates)) {
      ...
      for (seqfilenum in myseqfiles) {
        ...
        if (!file.exists(file.path(folder, efile)) |
            !file.exists(file.path(folder, mfile))) {
          ok <- FALSE
          attempt <- 1
          while (!ok) {
            unzip(
            ...
            if (!file.exists(file.path(folder, efile)) |
              ...
              if (file.info(file.path(folder, efile))$size == 0 |
                  file.info(file.path(folder, mfile))$size == 0) {
                if (!silent) {
                  cat('Problem: File size zero for ')
                  cat(efile)
                  cat(' or ')
                  cat(mfile)
                  cat('\n')
            ...
            if (attempt > attempts) {
              warning(
                '***Problem: Repeatedly failed to obtain valid data and/or moe file. May need to download/unzip manually. ***'
              )
              break
            }
            if (!ok) {
