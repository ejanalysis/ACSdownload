...
#' @return Side effect is unzipping file on disk
#' @seealso \code{\link{get.acs}}
#' @export
unzip.datafiles <- function(tables, mystates, folder=getwd()) {
    ...
    zipnames <- ""
    for (st in mystates) { zipnames <- c(zipnames, zipfile(st, seqfilelistnums))}
    zipnames <- zipnames[-1]
    ####################
    ...
    for (statenum  in 1:length(mystates)) {
      ...
      for (seqfilenum in myseqfiles) {
    		...
    		if (!testing) { 
    			...
    			if (!file.exists(efile) || !file.exists(mfile)) {
            ok <- FALSE; attempt <- 1
            while (!ok) {
              unzip(file.path(folder, zipfile(state.abbrev, seqfilenum)), files=c(file.path(folder, efile), file.path(folder, mfile)) ) 
              ...
              if (!file.exists(file.path(folder, efile)) || !file.exists(file.path(folder, mfile))) {
                ...
                if (file.info(file.path(efile))$size==0 || file.info(file.path(folder, mfile))$size==0) {
              ...
              if (!ok) { 
                ...
                cat('Retrying download of zip file since bad/missing contents after unzip attempt.\n')
                
                download.datafiles(tables, end.year=end.year, stateabbs[statenum], folder=folder) 
              }
            }
	...
	#################################
        ...
        #	****  requires "seqfilelistnums" in memory, datafile(), & mystates
        etxtnames <- ""
	...
	txtfilesinfolder <- dir(pattern="txt")
