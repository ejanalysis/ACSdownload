...
merge.tables <- function(my.list.of.tables) {
	...
	merged.tables <- my.list.of.tables[[1]][ order(my.list.of.tables[[1]]$KEY), dupecols]
	keycount <- length(merged.tables$KEY)
	
	for (i in 1:length(my.list.of.tables)) {
		if (length(my.list.of.tables[[i]]$KEY)!=keycount) { stop("Table lengths don't match")}
		othercols <- names(my.list.of.tables[[i]])[!(names(my.list.of.tables[[i]]) %in% dupecols)]		
		merged.tables <- cbind(merged.tables, my.list.of.tables[[i]][ order(my.list.of.tables[[i]]$KEY) , othercols])
	}
