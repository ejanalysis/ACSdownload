---
title: "Bulk-downloading ACS 5-year data"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Bulk-downloading ACS 5-year data}
...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  eval = FALSE  # network-dependent; do not run on CRAN
)
```

This vignette walks through the typical end-to-end use of
`ACSdownload::get_acs_new()`: which call shape to use, how to keep your
laptop healthy when fetching every block group in the country, and how
to turn the resulting cryptic column codes into readable labels.
...
* `yr = 2024` -- targets the 2020-2024 vintage (released 2026-01-29).
  Use `acs_endyear_like_ejam()` if you want today's "best guess" of the
...
requests, an API key, and rate limits. `get_acs_new()` does **16 large
file fetches** -- one per table -- and lets `data.table::fread()` parse
each one in a few seconds. On a residential connection a full block-
group pull is typically 3-10 minutes wall-clock, and ~zero on the
...
onto each blockgroup in the tract; if you need that behavior, fetch
them separately:
...
If the network is misbehaving, `get_acs_new()` retries each table on
HTTP 429 / 5xx with exponential backoff (default 3 retries). If 16
sequential downloads is too slow, opt into parallelism:
...
responsible for setting a `future::plan()`; without one, parallel runs
sequentially.

## Decoding the column names

The columns look like `B25034_001`, `B25034_002`, `B25034_M001`, ...
That's: table code, then `_<NNN>` for estimates or `_M<NNN>` for
margins of error. To turn those into something readable:

```{r}
acs_label(c("B25034_001", "B25034_002", "B25034_M002"))
```

`acs_label()` accepts either estimate or MOE column names; both map to
the same label. It returns NA for codes that don't appear in the
shipped lookup, which is built from the Census 2022 5-year table
shells.

## Picking just what you need

You don't always want every column from every table. `variables`
narrows the column selection at fetch time:
...
2. **Unknown table code?** `validate_acs_tables()` enforces
   `^[BC][0-9]{5}[A-I]?$`. Common typo: passing the EJSCREEN-style
